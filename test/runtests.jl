...
@test mixfrequencies(lf_data, hf_data, 3, 1, 1)[2].values[1, :] == [1.0, 7.0, 6.0, 5.0]
...
@test mixfrequencies(lf_data, hf_data, 3, 0, 0)[2].values[1, :] == [43545.0,43379.0,43396.0]
@test mixfrequencies(lf_data, hf_data, 3, 0, 0)[2].values[end, :] == [130974.0,130757.0,130563.0]

@test isapprox(collect(beta_weights_es(3, 1, 5)), [0.941176,0.0588238,9.4118e-25], rtol=1e-6)

y, x, yf, xf = mixfrequencies(lf_data, hf_data, 3, 0, 1)
